...
public:
	void Easing_strat(
		...
		double start,		//缓动开始值
		double end,			//缓动结束值
		void (*lpCbk)(double, double, int, int, bool&a, int, int),			//回调指针
		bool bThread,		//是否线程模式
		int param1,			//附加参数1
		int param2			//附加参数2
	) {
...
private:
	void Easing_Woking(int  uType, int dwTimer, int dwSleep, void (*lpCbk)(double,double,int,int,bool&a,int,int) , int param1, int param2, double start, double end, int addr) {
		int count;
		double pro;
		double cur;
		bool bStop = false;
		...
		for (int c = 0; c < count; c++)
		{
			...
			if ((int)lpCbk!=0){(*lpCbk)(pro, cur, count, i, bStop, param1, param2);}
	...
	double _EasingType_Elasticity(double percents_, double start, double end) {
		if (percents_ < 0) { percents_ = 0; }
		if (percents_ > 1) { percents_ = 1; }
		double _st = (sin(percents_ * PI * (0.2 + 2.5 * percents_ * percents_ * percents_)) * pow(1 - percents_, 2.2) + percents_) * (1 + 1.2 * (1 - percents_));
